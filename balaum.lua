...
-- @param string
-- @example
local function parse(____string)
    local ____TS__ArrayMap_result_0 = __TS__ArrayMap(
        __TS__StringSplit(
            __TS__StringTrim(____string),
            "\n"
        ),
        function(____, line) return __TS__StringTrim(line) end
    )
    local firstLine = ____TS__ArrayMap_result_0[1]
    local lines = __TS__ArraySlice(____TS__ArrayMap_result_0, 1)
    local columns = __TS__StringSplit(firstLine, ",")
    return __TS__ArrayMap(
        lines,
        function(____, line)
            local values = __TS__StringSplit(line, ",")
            return __TS__ObjectFromEntries(__TS__ArrayMap(
                values,
                function(____, value, index)
                    local column = columns[index + 1]
                    return {column, value}
                end
            ))
        end
...
local function stringify(data)
    local rows = __TS__ArrayIsArray(data) and data or ({data})
    if #rows == 0 then
        return ""
    end
    local columns = __TS__ObjectKeys(rows[1])
    return table.concat(
        __TS__ArrayMap(
            {
                columns,
                unpack(__TS__ArrayMap(
                    rows,
                    function(____, item) return __TS__ArrayMap(
                        columns,
                        function(____, column) return item[column] end
                    ) end
                ))
            },
            function(____, row) return __TS__ArrayJoin(row, ",") end
        ),
        "\n"
    )
end
____exports.parse = parse
____exports.stringify = stringify
return ____exports
 end,
["library.hook"] = function(...) 
local ____lualib = require("lualib_bundle")
...
local ____exports = {}
local functionMeta = __TS__New(WeakMap)
---
-- @param function_
-- @example
local function wrapFunction(function_)
    if functionMeta:has(function_) then
        for ____, ____value in __TS__Iterator(functionMeta.entries()) do
            local wrapper = ____value[1]
            local meta = ____value[2]
            if meta.orig == function_ then
                return wrapper
            end
        end
    end
    local meta = {breakpoints = {}, callbacks = {}, orig = function_}
    ...
    -- @example
    local function processBreakpoints(arguments_)
        local processed = arguments_
        for ____, breakpoint in ipairs(meta.breakpoints) do
            local result = breakpoint(unpack(processed))
            if result ~= nil then
                processed = __TS__ArrayIsArray(result) and result or ({result})
            end
        end
        return processed
    end
    ...
    -- @example
    local function processCallbacks(result, arguments_)
        local callbackArguments = result == nil and arguments_ or ({result})
        local output = {}
        for ____, callback in ipairs(meta.callbacks) do
            local callbackResult = callback(unpack(callbackArguments))
            if callbackResult ~= nil then
                output = __TS__ArrayIsArray(callbackResult) and callbackResult or ({callbackResult})
            end
        end
        return output
    end
    ...
    local function wrapper(...)
        local arguments_ = {...}
        local processedArguments = processBreakpoints(arguments_)
        local originalResult = meta.orig(unpack(processedArguments))
        local callbackOutput = processCallbacks(originalResult, processedArguments)
        if #callbackOutput > 0 then
            local ____temp_0
            if #callbackOutput == 1 then
                ____temp_0 = callbackOutput[1]
            else
                ____temp_0 = callbackOutput
            end
            return ____temp_0
        end
...
-- @param ephemeral
-- @example
local function addFunction(function_, ____type, hookFunction, ephemeral)
    if not hookFunction then
        return function_
    end
    local wrappedFunction = wrapFunction(function_)
    local meta = functionMeta:get(wrappedFunction)
    local hooksArray = meta[____type]
    ---
    -- @param hook
    -- @example
    local function addHook(hook)
        if ephemeral then
            hooksArray[#hooksArray + 1] = function(...)
                local hookResult = hook(...)
                if hookResult == nil or hookResult == true then
                    __TS__ArraySplice(
                        hooksArray,
                        __TS__ArrayIndexOf(hooksArray, hook),
                        1
                    )
                end
                return hookResult
...
-- @param ephemeral
-- @example
local function addBreakpoint(function_, breakpoint, ephemeral)
    if ephemeral == nil then
        ephemeral = false
    end
    return addFunction(function_, "breakpoints", breakpoint, ephemeral)
end
---
-- @param function_
-- @param callback
-- @param ephemeral
-- @example
local function addCallback(function_, callback, ephemeral)
    if ephemeral == nil then
        ephemeral = false
    end
    return addFunction(function_, "callbacks", callback, ephemeral)
end
---
-- @param function_
-- @example
local function isHooked(function_)
    return Boolean(
        nil,
        function_ and functionMeta:has(function_)
    )
end
---
-- @param function_
-- @example
local function clear(function_)
    if functionMeta:has(function_) then
        return functionMeta:get(function_).orig
    end
    return function_
end
local Hook = {addBreakpoint = addBreakpoint, addCallback = addCallback, clear = clear, isHooked = isHooked}
____exports.default = Hook
return ____exports
 end,
...
["library.server.handle-message.parse-content-parts"] = function(...) 
local ____exports = {}
local csv = require("library.csv")
---
-- @param contentParts
-- @example
local function parseContentParts(contentParts)
    if #contentParts > 0 then
        local content = table.concat(contentParts, "|")
        return csv.parse(content)
    end
    return {}
end
____exports.default = parseContentParts
return ____exports
 end,
["library.server.handle-message.start-run.click-main-menu-play-button"] = function(...) 
local ____exports = {}
---
-- @example
local function clickMainMenuPlayButton()
    if G.MAIN_MENU_UI then
        local playButton = G.MAIN_MENU_UI:get_UIE_by_ID("main_menu_play")
        if playButton then
...
____exports.default = clickMainMenuPlayButton
return ____exports
 end,
["library.server.handle-message.start-run.click-on-new-run"] = function(...) 
local ____exports = {}
---
-- @param deckName
-- @example
local function clickOnNewRun()
    local id = "tab_but_New Run"
    if G.OVERLAY_MENU then
        local tabButton = G.OVERLAY_MENU:get_UIE_by_ID(id)
        if tabButton then
...
____exports.default = clickOnNewRun
return ____exports
 end,
["library.server.handle-message.start-run.select-deck"] = function(...) 
local ____lualib = require("lualib_bundle")
...
-- @param deckName
-- @example
local function selectDeck(deckName)
    if deckName == nil then
        deckName = "Red Deck"
    end
    if G.P_CENTER_POOLS and G.P_CENTER_POOLS.Back then
        local deck = __TS__ArrayFind(
            __TS__ObjectValues(G.P_CENTER_POOLS.Back),
        ...
        local ____opt_0 = __TS__ArrayFind(
            __TS__ObjectEntries(G.P_CENTER_POOLS.Back),
            function(____, ____bindingPattern0)
                local name
                local key = ____bindingPattern0[1]
                name = ____bindingPattern0[2].name
                return name == deckName
            end
        )
        local deckKey = ____opt_0 and ____opt_0[1]
        if deck then
            G.FUNCS.change_viewed_back({to_key = deckKey, to_val = deck and deck.name})
            G.FUNCS.change_selected_back({to_key = deckKey})
            return true
        end
    end
    return false
end
____exports.default = selectDeck
return ____exports
 end,
["library.server.handle-message.start-run.start-new-run"] = function(...) 
local ____exports = {}
---
-- @param deckName
-- @example
local function startNewRun()
    ...
    G.FUNCS.start_run(nil, {challenge = _challenge, seed = _seed, stake = _stake})
    return true
end
____exports.default = startNewRun
return ____exports
 end,
["library.server.handle-message.start-run._exports"] = function(...) 
local ____exports = {}
do
    local ____click_2Dmain_2Dmenu_2Dplay_2Dbutton = require("library.server.handle-message.start-run.click-main-menu-play-button")
    ____exports.clickMainMenuPlayButton = ____click_2Dmain_2Dmenu_2Dplay_2Dbutton.default
end
do
    local ____click_2Don_2Dnew_2Drun = require("library.server.handle-message.start-run.click-on-new-run")
    ____exports.clickOnNewRun = ____click_2Don_2Dnew_2Drun.default
end
do
    local ____select_2Ddeck = require("library.server.handle-message.start-run.select-deck")
    ____exports.selectDeck = ____select_2Ddeck.default
end
do
    local ____start_2Dnew_2Drun = require("library.server.handle-message.start-run.start-new-run")
    ____exports.startNewRun = ____start_2Dnew_2Drun.default
end
return ____exports
 end,
["library.server.handle-message.start-run"] = function(...) 
local ____exports = {}
local ____queue = require("library.queue")
local queue = ____queue.default
local _____exports = require("library.server.handle-message.start-run._exports")
local clickMainMenuPlayButton = _____exports.clickMainMenuPlayButton
local clickOnNewRun = _____exports.clickOnNewRun
local selectDeck = _____exports.selectDeck
local startNewRun = _____exports.startNewRun
---
-- @param options0 - The root object
-- @param options0.deck - The root object
-- @example
local function startRun(____bindingPattern0)
    local deck
    deck = ____bindingPattern0.deck
    if deck == nil then
        deck = "Red Deck"
    end
    queue(
        {
            ...
            clickOnNewRun,
            function() return selectDeck(deck) end,
            startNewRun
        },
        1
    )
end
____exports.default = startRun
return ____exports
 end,
["library.server.handle-message._exports"] = function(...) 
local ____exports = {}
do
    local ____parse_2Dcontent_2Dparts = require("library.server.handle-message.parse-content-parts")
    ____exports.parseContentParts = ____parse_2Dcontent_2Dparts.default
end
do
    local ____start_2Drun = require("library.server.handle-message.start-run")
    ____exports.startRun = ____start_2Drun.default
end
return ____exports
 end,
...
local ____log = require("library.log")
local log = ____log.default
local _____exports = require("library.server.handle-message._exports")
local parseContentParts = _____exports.parseContentParts
local startRun = _____exports.startRun
---
-- @param message
-- @example
local function handleMessage(message)
    local ____TS__ArrayMap_result_0 = __TS__ArrayMap(
        __TS__StringSplit(message, "|"),
        function(____, part) return __TS__StringTrim(part) end
    )
    local ____type = ____TS__ArrayMap_result_0[1]
    local contentParts = __TS__ArraySlice(____TS__ArrayMap_result_0, 1)
    log(____type)
    repeat
        local ____switch4 = ____type
        local ____cond4 = ____switch4 == "startRun"
        if ____cond4 then
            do
                local result = parseContentParts(contentParts)
                local deck = result[1].deck
                startRun({deck = deck})
                break
            end
        end
...
local botHost = "0.0.0.0"
local botPort = 21122
local gameHost = "0.0.0.0"
local gamePort = 21121
local connection
local server
server = {
    ...
    sendMessage = function(message)
        ...
        if type(actualMessage) ~= "string" then
            actualMessage = __TS__ArrayJoin(
                {
                    actualMessage.type,
                    csv.stringify(actualMessage.content)
                },
                "|"
            )
        end
...
local ____exports = {}
____exports.csv = require("library.csv")
do
    local ____hook = require("library.hook")
    ____exports.Hook = ____hook.default
end
do
    local ____log = require("library.log")
...
local __TS__ArrayFind = ____lualib.__TS__ArrayFind
local ____exports = {}
local _____exports = require("library._exports")
local Hook = _____exports.Hook
local log = _____exports.log
local queue = _____exports.queue
local server = _____exports.server
                ...
                func = function()
                    ...
                    if state ~= currentState then
                        ...
                        if stateKey ~= nil then
                            ...
                            if server.isReady() then
                                local ____server_sendMessage_13 = server.sendMessage
                                local ____opt_2 = G.P_BLINDS
                                if ____opt_2 ~= nil then
                                    local ____opt_3 = G.GAME.round_resets.blind_choices
                                    if ____opt_3 ~= nil then
                                        ____opt_3 = ____opt_3.Small
                                    end
                                    ____opt_2 = ____opt_2[____opt_3]
                                end
                                local ____opt_6 = G.GAME.round_resets.blind_choices
                                if ____opt_6 ~= nil then
                                    ____opt_6 = ____opt_6.Small
                                end
                                local ____opt_10 = G.GAME.round_resets
                                if ____opt_10 ~= nil then
                                    ____opt_10 = ____opt_10.blind_tags
                                end
                                local ____opt_result_12
                                if ____opt_10 ~= nil then
                                    ____opt_result_12 = ____opt_10.Small
                                end
                                ____server_sendMessage_13({
                                    content = {
                                        blindChoice = ____opt_2,
                                        blindChoices = ____opt_6,
                                        blindTags = ____opt_result_12,
                                        state = tostring(stateKey)
                                    },
                                    type = "state"
                                })
                            end
...
local ____SMODS_14 = SMODS
local Atlas = ____SMODS_14.Atlas
Atlas({key = "modicon", path = "balaum.png", px = 26, py = 26})
return ____exports
 end,
["hook"] = function(...) 
 end,
}
return require("balaum", ...)
